...
function App() {
  ...
  const SignInClick = async (values) =>
  {
    ...
    var apiResponse = await fetch(addr+'/auth/login/', options).then((response) => 
    {
  ...
  return (
    <div className="App">
      {currentPage==="LoginPage" && <LoginPage onSignUpClick={() => setCurrentPage("RegisterPage")} onSignInClick={(values) => SignInClick(values)}/>}
      {currentPage==="RegisterPage" && <RegisterPage onBackClick={() => setCurrentPage("LoginPage")} onSignUpClick={(values) => SignUpClick(values)}/>}
      {currentPage==="MainMenu" && <MainMenu username={userName} onLogoutClick={() => LogoutClick()}/>}
      {currentPage==="ListEditPage" && <ListEditPage username="Matt" listName="ListA" onBackClick={() => setCurrentPage("MainMenu")}/>}
    </div>
  );
}
